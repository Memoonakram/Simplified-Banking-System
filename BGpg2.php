...
if($_SESSION['bank_guarantee_pg1'])
    $bank_guarantee_pg1_id = $_SESSION['bank_guarantee_pg1'];
else
    header("Location: BGpg1.php");
$bank_guarantee_pg2_id = 0;
...
$user_id = $_SESSION['id']; // Replace with dynamic user ID if applicable
$query = "SELECT * FROM bank_guarantee_pg2 WHERE bank_guarantee_pg1_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $bank_guarantee_pg1_id);
$stmt->execute();
...
if ($result->num_rows > 0) {
    ...
    $stmt = $conn->prepare("
        INSERT INTO bank_guarantee_pg2 (
            beneficiary_type, street, additional_address, postal_code, city, country, email, 
            mobile_number, relationship, contract_details, contact_person, bank_name, bank_street, 
            bank_additional_address, account_number, iban, swift, due_diligence, document_id,bank_guarantee_pg1_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
        $stmt->bind_param(
            ...
            $swift, $due_diligence, $document_id,$bank_guarantee_pg1_id
        );
        $stmt->execute();
        $bank_guarantee_pg2_id = $stmt->insert_id;
}
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $bank_additional_address = $_POST['additional-address'];
    ...
    if (isset($_FILES['document']) && $_FILES['document']['error'] == 0) {
        ...
        // Insert document as BLOB into documents table
        // $stmt = $conn->prepare("INSERT INTO documents (document_content, document_name,document_type,user_id) VALUES (?, ?,?,?)");
        // $stmt->bind_param("bssi", $fileContent, $fileName,$fileType,$user_id);
...
// Execute the SELECT query
    ...
    // Entry does not exist; proceed with the INSERT
        ...
        $stmt->bind_param(
            ...
            $email, $mobile_number, $relationship, $contract_details, $contact_person, 
            $bank_name, $bank_street, $bank_additional_address, $account_number, $iban, 
            $swift, $due_diligence, $document_id, $bank_guarantee_pg1_id, $bank_guarantee_pg2_id
        );
    ...
    //     );
    // }
    // Insert form data into pg2 table with reference to document_id

    if ($stmt->execute()) {
        // $sql = "UPDATE bank_guarantee_pg1 SET isSubmitted = TRUE WHERE id = ?";
        ...
        //     echo "error";
        echo "Form Submitted";
        header("Location: BG_submission.php");
    } else {
...
<head>
    ...
    <script>
        function goToNextPage() {
            // Navigate to bank_guarantee_submit.html
            window.location.href = "bank_guarantee_submit.html";
        }
    </script>
</head>
<body class="font-roboto bg-white text-gray-800">
  ...
  <span>Sign Out</span>
    ...
    <main class="p-8 max-w-4xl mx-auto">
        ...
        <form class="space-y-6" id="bgpg2form" action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <p class="mb-4">1. Please provide a copy of the contract for which the Bank Guarantee is being requested.</p>
            ...
            <div>
                ...
                <div>
                    <label class="block font-bold" for="additional-address">Additional Address</label>
                    <input class="border border-gray-300 rounded w-full px-3 py-2" name="additional-address" type="text"
                    value="<?php echo htmlspecialchars($additional_address); ?>"/>
                </div>
                <div>
    ...
    <footer class="bg-gray-800 text-white py-4 mt-8">
        <div class="max-w-4xl mx-auto flex justify-between items-center">
            ...
            <div>
                <img alt="Bank logo" class="h-10 w-10" src="https://storage.googleapis.com/a1aa/image/GaIx7eLNp1xGV64RrYNs52VGMx4e3PCNILK5rfxJY0crkEanA.jpg"/>
            </div>
        </div>
    </footer>
    <script>
...
document.addEventListener("DOMContentLoaded", () => {
  setupChangeAutoSubmit("bgpg2form"); // Replace "bgpg1form" with the ID of your form
});

...
$expiryDate = '';$amount = '';$currency = '';$is_edit = false;$message = "";

$query = "SELECT * FROM lc_pg1 WHERE user_id = ? ORDER BY created_at desc";
$stmt = $conn->prepare($query);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $sql = "UPDATE lc_pg1 SET isSubmitted = TRUE WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $lc_pg1_id);
    if($stmt->execute()){
        unset($_SESSION['lc_pg1_id']);
        header("Location: lc.php");
        echo "form submitted";
    }
    else
...
<html>
  ...
  <span>Sign Out</span>
            ...
            <div class="text-red-600 mb-4">
                This data is collected for the application “lc Aval”.
            </div>
            </div>
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="flex items-center text-red-600">
            <a href="lc.php" class="flex items-center text-red-600">
        ...
        <div class="ml-4">
          <p class="mb-1">
            <span class="font-semibold">Transferable: </span><?php echo htmlspecialchars($transferable); ?>
          </p>
          <p class="mb-1">
          ...
          <p class="mb-1">
            <span class="font-semibold">Payment at Sight: </span><?php echo htmlspecialchars($payment_atSight); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Incoterm: </span><?php echo htmlspecialchars($incoterm); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Type: </span><?php echo htmlspecialchars($type); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Partial Shipment: </span><?php echo htmlspecialchars($partialShipment); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Trans Shipment: </span><?php echo htmlspecialchars($transshipment); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Port of Loading/ Airport of Departure/ Place of Taking in Charge: </span><?php echo htmlspecialchars($loadingPort); ?>"
          </p>
          <p class="mb-1">
            <span class="font-semibold">Port of Discharge/ Airport of Destination/ Place of Final Destination: </span><?php echo htmlspecialchars($dischargePort); ?>"
          </p>
          <p class="mb-1">
            <span class="font-semibold">Latest Date of Shipment: </span><?php echo htmlspecialchars($latestDate); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Certificate of Origin: </span><?php echo htmlspecialchars($certificate); ?>
          </p>
          <h3>DETAILS OF CONTRACT IN RESPECT OF WHICH THE LC IS REQUESTED</h3>
          <p class="mb-1">
            <span class="font-semibold">Contract Numbers: </span><?php echo htmlspecialchars($contractNums); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Porforma Invoice Numbers: </span><?php echo htmlspecialchars($proformaNums); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Shipping Marks: </span><?php echo htmlspecialchars($shippingMarks); ?>
          </p>
          <p class="mb-1">
            <span class="font-semibold">Special Conditions: </span><?php echo htmlspecialchars($specialConditions); ?>
          </p>
        </div>
      </div>
      <div>
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="flex items-center text-red-600">
            <a href="lcpg2.php" class="flex items-center text-red-600">
                <span class="mr-1">Edit</span>
                <i class="fas fa-pencil-alt"></i>
            </a>
          </div>

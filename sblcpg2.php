...
if($_SESSION['sblc_pg1_id'])
    $sblc_pg1_id = $_SESSION['sblc_pg1_id'];
else
    header("Location: sblc.php");
$sblc_pg2_id = 0;
...
if ($result->num_rows > 0) {
    ...
    $stmt = $conn->prepare("
        INSERT INTO sblc_pg2 (
            beneficiary_type, street, additional_address, postal_code, city, country, email, 
            mobile_number, relationship, contract_details, contact_person, bank_name, bank_street, 
            bank_additional_address, account_number, iban, swift, due_diligence, document_id,sblc_pg1_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
        $stmt->bind_param(
            ...
            $swift, $due_diligence, $document_id,$sblc_pg1_id
        );
        $stmt->execute();
        $sblc_pg2_id = $stmt->insert_id;
}
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt->bind_param(
        ...
        $email, $mobile_number, $relationship, $contract_details, $contact_person, 
        $bank_name, $bank_street, $bank_additional_address, $account_number, $iban, 
        $swift, $due_diligence, $document_id, $sblc_pg1_id, $sblc_pg2_id
    );
    
    if ($stmt->execute()) {
        echo "Form Submitted";
        header("Location: sblc_submission.php");
    } else {

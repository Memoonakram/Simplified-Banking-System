<?php
  ...
  $query = "SELECT * FROM warranty_pg1 WHERE user_id = ? AND isSubmitted = FALSE ORDER BY created_at DESC";
  ...
  if ($result->num_rows > 0) {
    ...
    $insert_query = "INSERT INTO warranty_pg1 (user_id, seller_code, buyer_code,contract_no,expiry_date, amount, currency) VALUES (?, ?, ?, ?, ?,?,?)";
    $stmt = $conn->prepare($insert_query);
    $stmt->bind_param("issssds", $user_id, $seller_code, $buyer_code, $contract_no, $expiryDate, $amount, $currency);
    $stmt->execute();
    $warranty_pg1_id = $stmt->insert_id;
    $_SESSION['warranty_pg1_id'] = $warranty_pg1_id;
  }
  $stmt->close();
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_query = "UPDATE warranty_pg1 SET seller_code = ?, buyer_code = ?, contract_no = ?, expiry_date = ?, amount = ?, currency = ? WHERE id = ?";
    $stmt = $conn->prepare($update_query);
    $stmt->bind_param("ssssdsi", $seller_code, $buyer_code, $contract_no,$expiryDate, $amount, $currency, $warranty_pg1_id);

    // Execute the statement and show a message
    if ($stmt->execute()) {
        $_SESSION['warranty_pg1_id'] = $warranty_pg1_id;
        header("Location: warrantypg2.php");
  ...
  <span>Sign Out</span>
    ...
    <nav class="bg-gray-800 text-white py-4 w-full">
      <div class="max-w-4xl mx-auto flex justify-between items-center">
        <div class="text-lg font-bold">TRADE FINANCE – WARRANTY</div>
      </div>
    </nav>
    ...
    <main class="max-w-4xl mx-auto p-4 mt-8">
      ...
      <div class="text-red-600 mb-4">
        This data is collected for the application “Warranty”.
      </div>
      <div class="text-2xl font-bold mb-4">
        TERMS OF PROOF OF FUNDS (“AVAL”)
      </div>
      <form method="post" id="warrantypg1form">
        <!-- Transferable Field -->
    ...
    <div class="fixed bottom-4 right-4">
      <img
        alt="Company logo"
        height="50"
        src="https://storage.googleapis.com/a1aa/image/fSiYnEV7C8zZPCl9MAfm3T7b4GY4iAsQjJAJPtMx1mtsqmsTA.jpg"
        width="50"
      />
    </div>
    ...
    setupChangeAutoSubmit("warrantypg1form"); // Replace "bgpg1form" with the ID of your form
  });

...
if ($result->num_rows > 0) {
    ...
    $stmt = $conn->prepare("
        INSERT INTO warranty_pg2 (
            beneficiary_type, street, additional_address, postal_code, city, country, email, 
            mobile_number, relationship, contract_details, contact_person, bank_name, bank_street, 
            bank_additional_address, account_number, iban, swift, due_diligence, document_id,warranty_pg1_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
        $stmt->bind_param(
            ...
            $swift, $due_diligence, $document_id,$warranty_pg1_id
        );
        $stmt->execute();
        $warranty_pg2_id = $stmt->insert_id;
}
$stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt->bind_param(
        ...
        $email, $mobile_number, $relationship, $contract_details, $contact_person, 
        $bank_name, $bank_street, $bank_additional_address, $account_number, $iban, 
        $swift, $due_diligence, $document_id, $warranty_pg1_id, $warranty_pg2_id
    );
    
    if ($stmt->execute()) {
        echo "Form Submitted";
        header("Location: warranty_submission.php");
    } else {
...
<body class="font-roboto bg-white text-gray-800">
  ...
  <span>Sign Out</span>
    ...
    <main class="p-8 max-w-4xl mx-auto">
        ...
        <form class="space-y-6" id="warrantypg2form" action="" method="POST" enctype="multipart/form-data">
...
document.addEventListener("DOMContentLoaded", () => {
  setupChangeAutoSubmit("warrantypg2form"); // Replace "bgpg1form" with the ID of your form
});
